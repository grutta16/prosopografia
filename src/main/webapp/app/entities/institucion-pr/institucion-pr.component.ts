...
@Component({
    selector: 'jhi-institucion-pr',
    templateUrl: './institucion-pr.component.html'
})
export class InstitucionPrComponent implements OnInit, OnDestroy {
...
currentAccount: any;
    ...
    loadAll() {
        if (this.currentSearch) {
            this.institucionService.search({
                page: this.page - 1,
                query: this.currentSearch,
                size: this.itemsPerPage,
                sort: this.sort()}).subscribe(
                    (res: HttpResponse<InstitucionPr[]>) => this.onSuccess(res.body, res.headers),
    ...
    transition() {
        this.router.navigate(['/institucion-pr'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.currentSearch = '';
        this.router.navigate(['/institucion-pr', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: InstitucionPr) {
        return item.id;
    }
