...
@Component({
    selector: 'jhi-provincia-pr',
    templateUrl: './provincia-pr.component.html'
})
export class ProvinciaPrComponent implements OnInit, OnDestroy {
...
currentAccount: any;
    ...
    loadAll() {
        if (this.currentSearch) {
            this.provinciaService.search({
                page: this.page - 1,
                query: this.currentSearch,
                size: this.itemsPerPage,
                sort: this.sort()}).subscribe(
                    (res: HttpResponse<ProvinciaPr[]>) => this.onSuccess(res.body, res.headers),
    ...
    transition() {
        this.router.navigate(['/provincia-pr'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.currentSearch = '';
        this.router.navigate(['/provincia-pr', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: ProvinciaPr) {
        return item.id;
    }
