...
@Component({
    selector: 'jhi-persona-pr',
    templateUrl: './persona-pr.component.html'
})
export class PersonaPrComponent implements OnInit, OnDestroy {
...
currentAccount: any;
        ...
        if (this.currentSearch) {
            this.personaService.search({
                ...
                sort: this.sort()}).subscribe(
                    (res: HttpResponse<PersonaPr[]>) => this.onSuccess(res.body, res.headers),
    ...
    transition() {
        this.router.navigate(['/persona-pr'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.currentSearch = '';
        this.router.navigate(['/persona-pr', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: PersonaPr) {
        return item.id;
    }
